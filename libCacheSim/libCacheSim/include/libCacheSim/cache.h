...
// Helper to print all object IDs in the cache (for LRU-style caches)
static inline void print_cache_obj_ids(const cache_t *cache, FILE *out) {
    if (!cache || !cache->eviction_params || !out) return;
    
    // Try to safely access q_head for LRU-style algorithms
    // For LRU-style algorithms, q_head is typically the first member of eviction_params
    void **params = (void **)cache->eviction_params;
    if (params && params[0]) {
        cache_obj_t *cur = (cache_obj_t *)params[0];
        int count = 0;
        const int max_count = 1000000; // Safety limit
    ...
    // If the structure doesn't match, just skip (no error)
}
