...
enum argp_option_short {
  ...
  OPTION_DUMP_CACHE_OBJ_IDS = 0x200,
};
...
static struct argp_option options[] = {
    ...
    {"dump-cache-obj-ids", OPTION_DUMP_CACHE_OBJ_IDS, "FILENAME", 0, "Dump final cache object IDs to file (or '-' for stdout)", 6},
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
  ...
  switch (key) {
    ...
    case OPTION_WARMUP_SEC:
      ...
      break;
    case OPTION_DUMP_CACHE_OBJ_IDS:
      if (arg) {
        strncpy(arguments->dump_cache_obj_ids, arg, 255);
        arguments->dump_cache_obj_ids[255] = '\0';
      }
      break;
    case ARGP_KEY_ARG:
