...
int main(int argc, char **argv) {
  ...
  if (args.n_cache_size * args.n_eviction_algo == 1) {
    ...
    // PATCH: Dump cache object IDs if requested
    if (args.dump_cache_obj_ids[0]) {
      FILE *out = NULL;
      if (strcmp(args.dump_cache_obj_ids, "-") == 0) {
        out = stdout;
      } else {
        out = fopen(args.dump_cache_obj_ids, "w");
        if (!out) {
          fprintf(stderr, "Failed to open %s for writing\n", args.dump_cache_obj_ids);
          free_arg(&args);
          return 1;
        }
      }
      print_cache_obj_ids(args.caches[0], out);
