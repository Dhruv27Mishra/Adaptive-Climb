...
int main(int argc, char **argv) {
  ...
  cache_t *ilru = iLRU_init(cc_params, NULL);

  int64_t n_hit = 0, n_req = 0;
  int64_t n_hit_ilru = 0;
  while (read_one_req(reader, req) == 0) {
    if (lru->get(lru, req)) {
      n_hit++;
    }
    if (ilru->get(ilru, req)) {
      n_hit_ilru++;
    }
    n_req++;
  }

  printf("LRU hit ratio: %lf\n", (double)n_hit / n_req);
  printf("iLRU hit ratio: %lf\n", (double)n_hit_ilru / n_req);

  free_request(req);
  lru->cache_free(lru);
  ilru->cache_free(ilru);
  close_reader(reader);
